...
module.exports = function staticCache(dir, options, files) {
  ...
  return async function mystatic(ctx, next) {
    ...
    if (file.type == 'text/html') {
      let htmlBuff = await fs.readFileSync(file.path);
      // let stream = await fs.createReadStream(file.path);
      var stream = createIp(htmlBuff, ctx.app.config.site.API)
    } else {
...
function createIp(htmlBuff, API) {
  // let insert = `<!DOCTYPE html><head><script>window.rayplus_ip = '${API}'</script>`
  // let insertBuff = Buffer.allocUnsafe(insert.length).fill(insert);
  // let len = htmlBuff.length + insert.length;
  // return Buffer.concat([insertBuff, htmlBuff], len);
  return htmlBuff.toString().replace(/(\<body.*?\>)/, `$1<script>window.rayplus_ip = '${API}'</script>`);
}